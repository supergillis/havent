...
### The camera is picky about the offer it will answer

*Established 2026-08-09 while replacing the Go bridge with a Python signaling relay (see
`docs/superpowers/specs/2026-08-09-containerless-camera-design.md`), by feeding the camera offers
generated by a stack other than this project's own — go2rtc's WebRTC client.*

The camera does not answer an arbitrary well-formed offer. Three constraints, none of them
advertised anywhere in the API:

1. **Audio must be the first m-line.** This is the sharp one. Given a **video-first** offer, the
   camera does not reject it — it answers with an **empty video section**, which is far harder to
   diagnose:

   ```
   m=video 9 UDP/TLS/RTP/SAVPF 96
   c=IN IP4 0.0.0.0m=audio 9 UDP/TLS/RTP/SAVPF 0     ← note the missing CRLF
   c=IN IP4 0.0.0.0
   a=ice-ufrag:...                                    ← the audio section, complete
   ```

   The video m-line is emitted with the negotiated payload type and then nothing: no `mid`, no ICE
   credentials, no `rtpmap`, and the CRLF before the next m-line is lost. Reordering the offer to
   audio-then-video produces a complete, correct two-section answer. This project's bridge has
   always offered audio first (`pkg/rtsp/bridge.go`, `medias := []{audio, video}`), which is why it
   never hit this; anyone reusing a general-purpose WebRTC client will, since offering video first
   is the common convention.

2. **One audio and one video section, nothing else.** A third m-line (go2rtc offers one for its
   audio backchannel) is simply not represented in the answer, so the extra section has to be
   dropped before sending and answered as rejected (port 0) on the way back.

3. **No `a=extmap`.** Stripping these was already in the Go bridge, attributed to a payload-size
   limit. The behaviour is consistent with a fixed-size SDP buffer on the device, which is also the
   most plausible explanation for the truncation in (1).

The answer itself is well behaved: it echoes the offered `mid` values, so sections can be paired by
`mid` rather than position, and it comes back in a single MQTT frame (~1.8 kB) within **~0.12 s**,
followed by host, srflx and TURN-relay candidates within another ~150 ms.

One asymmetry worth noting: the camera answers its audio section `a=sendrecv` even when the offer
said `recvonly`. That is protocol-invalid but harmless — `pion` clamps it — and it is a hint that
the speaker path is always live on the device side (see issue #72, where an actually-sendrecv offer
makes the camera take its speaker and stop a playing lullaby).

### The session pool: `max_client_num`, zombies, and `disconnect` scope

*Established 2026-08-09 during the first field trial of the Python signaling relay (see
`docs/superpowers/specs/2026-08-09-containerless-camera-design.md`), verified against the trial
logs, go2rtc's source and Tuya's TuyaOS IPC firmware documentation — [Live Preview and Two-Way
Talk](https://developer.tuya.com/en/docs/iot-device-dev/tuyaos-package-ipc-device?id=Kcn1px33iptn2)
and [Real-Time
Transmission](https://developer.tuya.com/en/docs/iot-device-dev/realtime_map_route_trans_0_0?id=Ke95caumzqd6y).*

The camera serves a small fixed number of concurrent WebRTC sessions — `max_client_num` in TuyaOS
IPC firmware, range 0–5 with **3–5 recommended**. (That figure is vendor documentation; the value
baked into the SCD9xx firmware was not measured here, but the field behaviour fits the documented
range.) Three behaviours follow, none advertised in the API:

1. **A full pool closes the newcomer, not the oldest session.** The dial fails — the vendor app
   surfaces it as "Device Busy", the firmware logs `incoming connection refused` — while
   established sessions keep streaming. There is no queueing and no eviction.
2. **Abandoned sessions become zombies.** A session whose handshake was started and never
   completed, or whose peer vanished without a `disconnect`, is reclaimed only by a firmware
   inactivity timer (`tuya_ipc_p2p_set_zombie_session_timeout`, default **20 minutes**). A client
   that opens sessions faster than it disconnects them will lock everyone out of the camera,
   including the vendor app. (Home Assistant's 10 s thumbnail poll, routed through go2rtc's
   dial-grab-stop `/api/frame.jpeg` handler, opened ~360 sessions/hour and did exactly that; once
   the churn stopped, the pool took roughly ten minutes of retrying to come back, with the vendor
   app locked out throughout.)
3. **The `disconnect` frame is session-scoped.** It carries the sender's `sessionid` (the frame
   format above) and ends that session only. Session ids are per-session; the camera sends
   per-session disconnects. Both go2rtc's `pkg/tuya` and this project's bridge
   (`pkg/rtsp/bridge.go`, `Stop()`) send it for their own session id at their own teardown, body
   `{"mode": "webrtc"}` on protocol 302. Sending it promptly for every session you abandon is what
   keeps the pool healthy; there is no way to free someone else's slot, and no need to.

When the pool has free slots the camera answers an offer in **74–135 ms**; when it is full it
answers with silence. Clients should treat a short answer timeout (a few seconds) plus a back-off
of tens of seconds as protocol, not tuning.
